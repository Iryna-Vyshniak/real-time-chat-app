...
const GroupFullInfo = ({
  ...
  const adminName = data.participantsData
    ? data.participantsData.find((participant) => participant._id === data.groupAdmin)
    : data.participants?.find((participant) => participant._id === data.groupAdmin._id);

  const isAdmin = adminName?.fullName === authUser.fullName;

  const avatars = selectedUsers.map((participant) => participant);
